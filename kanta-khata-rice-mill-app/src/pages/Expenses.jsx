...
export default function Expenses() {
  ...
  async function submitMachine(e) {
    e.preventDefault();
    const { error } = await machinery.insert({
  ...
  async function submitExpense(e) {
    ...
    const { error } = await expenses.insert({
      date: eForm.date, category: eForm.category, description: eForm.description,
      amount: Number(eForm.amount), paid_via: eForm.paid_via,
    });
    if (error) { alert('Save nahi hua: ' + error.message); return; }
  ...
  return (
    <>
      ...
      {writable && (
        <Panel title="Machine Maintenance Log — Nayi Entry">
          <form onSubmit={submitMachine}>
            ...
            <button type="submit" className="btn primary">Log Save Karein</button>
          </form>
        </Panel>
      )}
      ...
      <Panel title="Machinery Log" action={<button className="btn small" onClick={() => exportCSV('machinery_log.csv', ['Date','Machine','Issue','Action','Cost','NextService','Status'], machinery.rows.map((m) => [m.date, m.machine, m.issue_description, m.action_taken, m.cost, m.next_service_due, m.status]))}>Export CSV</button>}>
        <div className="tablewrap">
          <table className="data">
            <thead><tr><th>Date</th><th>Machine</th><th>Issue</th><th>Action Taken</th><th>Cost</th><th>Next Service</th><th>Status</th>{writable && <th></th>}</tr></thead>
            <tbody>
              {machinery.rows.length ? machinery.rows.map((m) => (
                <tr key={m.id}>
                  ...
                  {writable && <td><button className="btn small danger" onClick={() => machinery.remove(m.id)}>Delete</button></td>}
                </tr>
      ...
      {writable && (
        <Panel title="Daily Expense — Nayi Entry">
          <form onSubmit={submitExpense}>
            ...
            <button type="submit" className="btn primary">Expense Save Karein</button>
          </form>
        </Panel>
      )}
      ...
      <Panel title="Expense Log" action={<button className="btn small" onClick={() => exportCSV('expenses.csv', ['Date','Category','Description','Amount','PaidVia'], expenses.rows.map((x) => [x.date, x.category, x.description, x.amount, x.paid_via]))}>Export CSV</button>}>
        <div className="tablewrap">
          <table className="data">
            <thead><tr><th>Date</th><th>Category</th><th>Description</th><th>Amount</th><th>Paid Via</th>{writable && <th></th>}</tr></thead>
            <tbody>
              {expenses.rows.length ? expenses.rows.map((x) => (
                <tr key={x.id}>
                  ...
                  {writable && <td><button className="btn small danger" onClick={() => expenses.remove(x.id)}>Delete</button></td>}
                </tr>
